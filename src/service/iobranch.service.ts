import { CreateIobranchDto, UpdateIobranchDto, IobranchDto, FindbranchByIdDto } from 'src/dto/iobranch.dto';
...
@Injectable()
export class IobranchService {
  constructor(private dataSource: DataSource) {}

  async findbranchById(dto: FindbranchByIdDto): Promise<any> {
    try {
      ...
      // Add full image URL if branch_image exists
      const imageBaseUrl = `${process.env.IMAGE_BASE_URL}/iouser/`;
      const branchWithImageUrl = result.map((branch: any) => ({
        ...branch,
  ...
  async findbranchsByStatus(dto: IobranchDto): Promise<any> {
    try {
      ...
      const branchsWithImageUrls = result.map((branch: any) => ({
        ...branch,
        image_url: branch.branch_image ? imageBaseUrl + branch.branch_image : null,
      }));
  ...
  async addbranchWithImage(branchDto: any): Promise<{ status: string; message: string; data?: any }> {
    try {
      ...
      // Branch code validation - check if branch_code already exists
      if (branch_code) {
  ...
  async updatebranchWithImage(
    ...
    try {
      ...
      const [existingbranch] = await this.dataSource.query(
        `SELECT branch_id, branch_name, branch_code, branch_image FROM io_branch WHERE branch_id = ?`,
        [branchId],
      );
      ...
      if (result.affectedRows === 0) {
        ...
        throw new NotFoundException('branch not found or no changes made');
      }
  ...
  async deletebranch(branchId: number): Promise<{ status: string; message: string }> {
    try {
      console.log(`🗑️ DELETE BRANCH REQUEST for ID: ${branchId}`);
      
      // Check if branch exists
      const [existingbranch] = await this.dataSource.query(
        `SELECT branch_id, branch_name, branch_code FROM io_branch WHERE branch_id = ?`,
        [branchId],
      );
