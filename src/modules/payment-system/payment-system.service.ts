// src/modules/payment-system/payment-system.service.ts
...
export class PaymentSystemService {
  ...
  // =====================================================
  // ACQUIRER SETTLEMENT METHODS
  // =====================================================

  /**
   * Create new acquirer settlement record
   */
  async createAcquirerSettlement(dto: CreateAcquirerSettlementDto): Promise<any> {
    try {
      const query = `
  ...
  /**
   * Get acquirer settlement records with filters
   */
  async getAcquirerSettlements(filters: FilterDto = {}): Promise<any> {
    try {
  ...
  // =====================================================
  // PSP RECONCILIATION METHODS
  // =====================================================

  /**
   * Create new PSP reconciliation record
   */
  async createPspReconciliation(dto: CreatePspReconciliationDto): Promise<any> {
    try {
      const query = `
  ...
  /**
   * Get PSP reconciliation records with filters
   */
  async getPspReconciliations(filters: FilterDto = {}): Promise<any> {
    try {
  ...
  /**
   * Create new settlement summary record
   */
  async createSettlementSummary(dto: CreateSettlementSummaryDto): Promise<any> {
    try {
      const query = `
  ...
  /**
   * Get settlement summary records with filters
   */
  async getSettlementSummaries(filters: FilterDto = {}): Promise<any> {
    try {
  ...
  /**
   * Create new transaction details record
   */
  async createTransactionDetails(dto: CreateTransactionDetailsDto): Promise<any> {
    try {
      const query = `
  ...
  /**
   * Get transaction details records with filters
   */
  async getTransactionDetails(filters: FilterDto = {}): Promise<any> {
    try {
  ...
  /**
   * Import Acquirer Settlement data from CSV
   */
  async importAcquirerSettlementCSV(csvData: any[]): Promise<any> {
    try {
      const results = [];
      const errors = [];
      ...
      for (let i = 0; i < csvData.length; i++) {
        try {
          const row = csvData[i];
          const dto: CreateAcquirerSettlementDto = {
  ...
  /**
   * Import PSP Reconciliation data from CSV
   */
  async importPspReconciliationCSV(csvData: any[]): Promise<any> {
    try {
      const results = [];
      const errors = [];
      ...
      for (let i = 0; i < csvData.length; i++) {
        try {
          const row = csvData[i];
          const dto: CreatePspReconciliationDto = {
  ...
  /**
   * Import Settlement Summary data from CSV
   */
  async importSettlementSummaryCSV(csvData: any[]): Promise<any> {
    try {
      const results = [];
      const errors = [];
      ...
      for (let i = 0; i < csvData.length; i++) {
        try {
          const row = csvData[i];
          const dto: CreateSettlementSummaryDto = {
  ...
  /**
   * Import Transaction Details data from CSV
   */
  async importTransactionDetailsCSV(csvData: any[]): Promise<any> {
    try {
      const results = [];
      const errors = [];
      ...
      for (let i = 0; i < csvData.length; i++) {
        try {
          const row = csvData[i];
          const dto: CreateTransactionDetailsDto = {
   ...
   */
  async getReconciliationReport(filters: FilterDto = {}): Promise<any> {
    try {
      let query = `
        SELECT 
          source_type,
   ...
   */
  async getDailySummary(filters: FilterDto = {}): Promise<any> {
    try {
      let query = `
        SELECT 
   ...
   */
  async getMerchantSummary(merchant_id: string, filters: FilterDto = {}): Promise<any> {
    try {
      let query = `
        SELECT 
          merchant_id,
  ...
  /**
   * Get actual table structure for debugging
   */
  ...
  /**
   * Validate system transaction ID exists in reconciliation view
   */
  async validateTransactionExists(system_txn_id: string): Promise<any> {
    try {
      const query = `
        SELECT 
          primary_id,
  ...
  /**
   * Get transaction count by table
   */
