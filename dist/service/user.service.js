...
let userService = class userService {
    ...
    async findAllCustomer() {
        try {
            let query = `SELECT * FROM kdv_customer_address`;
            const params = [];
            const result = await this.dataSource.query(query, params);
            const imageBaseUrl = `${process.env.IMAGE_BASE_URL}/customer/`;
            const customersWithPhotos = result.map((user) => ({
                ...user,
                photo: user.photo ? imageBaseUrl + user.photo : null,
                photo_id: user.photo_id ? imageBaseUrl + user.photo_id : null,
            }));
    ...
    async findAllDriver() {
        try {
            let query = `SELECT * FROM kdv_Driver_address`;
            const params = [];
            const result = await this.dataSource.query(query, params);
            const imageBaseUrl = `${process.env.IMAGE_BASE_URL}/driver/`;
            const DriversWithPhotos = result.map((user) => ({
                ...user,
                photo: user.photo ? imageBaseUrl + user.photo : null,
                photo_id: user.photo_id ? imageBaseUrl + user.photo_id : null,
            }));
    ...
    async addUserWithPhoto(userDto) {
        try {
            const { name, username, email, password, phone, document_id, photo, photo_id, village_id, district_id, province_id, status, role, account_bank_id, account_no, account_name, language, bio, online, } = userDto;
    ...
    async updateUserWithPhoto(phone, userDto) {
        try {
            console.log('📝 Received userDto:', JSON.stringify(userDto, null, 2));
            const { name, username, email, phone: newPhone, document_id, photo, photo_id, village_id, district_id, province_id, status, role, account_bank_id, account_no, account_name, language, online, bio, } = userDto;
            const [existingUser] = await this.dataSource.query(`SELECT photo, photo_id FROM kd_user WHERE phone = ?`, [phone]);
            if (!existingUser) {
    ...
    async updateDriver(phone, driverDto) {
        try {
            ...
            const { name, username, email, phone: newPhone, document_id, photo, photo_id, village_id, district_id, province_id, status, role, account_bank_id, account_no, account_name, language, online, bio, } = driverDto;
            const [existingDriver] = await this.dataSource.query(`SELECT photo, photo_id FROM kd_driver WHERE phone = ?`, [phone]);
            if (!existingDriver) {
    ...
    async findDriverByPhone(phone) {
        try {
            ...
            const imageBaseUrl = `${process.env.IMAGE_BASE_URL}/driver/`;
            const DriversWithPhotos = result.map((driver) => ({
                ...driver,
    ...
    async updateCustomer(phone, userDto) {
        try {
            console.log('📝 Received userDto:', JSON.stringify(userDto, null, 2));
            const { name, username, email, phone: newPhone, document_id, photo, photo_id, village_id, district_id, province_id, status, role, account_bank_id, account_no, account_name, language, online, bio, } = userDto;
            const [existingUser] = await this.dataSource.query(`SELECT photo, photo_id FROM kd_customer WHERE phone = ?`, [phone]);
            if (!existingUser) {
                throw new Error('User not found');
            }
            const photoFilename = photo ? await this.saveImage(photo) : existingUser.photo;
    ...
    async saveProfileImage(base64Str) {
        try {
    ...
    async getProfileImageByCustomerId(user) {
        const { phone, role } = user;
        try {
            const result = await this.dataSource.query(`SELECT * FROM kdv_profile WHERE phone = ? ORDER BY profile_date DESC LIMIT 1`, [phone]);
            if (result.length === 0) {
                return {
                    status: 'not_found',
    ...
    async getProfiledriver(body) {
        const { phone, role } = body;
        try {
            const result = await this.dataSource.query(`SELECT * FROM kdv_dvprofile WHERE driver_phone = ? ORDER BY profile_date DESC LIMIT 1`, [phone]);
            if (result.length === 0) {
                return {
                    status: 'not_found',
