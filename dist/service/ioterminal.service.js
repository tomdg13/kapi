...
exports.IoTerminalService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("typeorm");
const fs = require("fs/promises");
const path = require("path");
const crypto = require("crypto");
let IoTerminalService = class IoTerminalService {
    ...
    async findTerminalById(dto) {
        try {
            ...
            const terminalWithUrls = result.map((terminal) => ({
                ...terminal,
    ...
    async findTerminalsByStatus(dto) {
        try {
            let query;
            let params = [];
            ...
            const { imageBaseUrl, pdfBaseUrl } = this.getBaseUrls();
            const terminalsWithUrls = result.map((terminal) => ({
                ...terminal,
                image_url: terminal.terminal_image ? imageBaseUrl + terminal.terminal_image : null,
                pdf_url: terminal.terminal_pdf ? pdfBaseUrl + terminal.terminal_pdf : null,
            }));
    ...
    async generateTerminalCode(companyId, userId) {
        try {
    ...
    async addTerminalWithImage(terminalDto) {
        try {
            const { store_id = 1, merchant_id, group_id, company_id, terminal_name, phone, serial_number, sim_number, expire_date, image, terminal_pdf, pdf_filename, user_id, } = terminalDto;
      ...
      `;
            ...
            const createdTerminal = await this.dataSource.query(`SELECT * FROM io_terminal WHERE terminal_id = ?`, [result.insertId]);
    ...
    async updateTerminalWithImage(terminalId, terminalDto) {
        try {
            console.log('Received terminalDto:', JSON.stringify(terminalDto, null, 2));
            const { store_id, merchant_id, group_id, company_id, terminal_name, terminal_code, phone, serial_number, sim_number, expire_date, image, terminal_pdf, pdf_filename, create_by } = terminalDto;
            const [existingTerminal] = await this.dataSource.query(`SELECT * FROM io_terminal WHERE terminal_id = ?`, [terminalId]);
            if (!existingTerminal) {
      ...
      `;
            ...
            if (result.affectedRows === 0) {
                throw new common_1.NotFoundException('Terminal not found or no changes made');
            }
            const [updatedTerminal] = await this.dataSource.query(`SELECT * FROM io_terminal WHERE terminal_id = ?`, [terminalId]);
    ...
    async deleteTerminal(terminalId) {
        try {
            const [existingTerminal] = await this.dataSource.query(`SELECT terminal_id, terminal_name, terminal_image, terminal_pdf FROM io_terminal WHERE terminal_id = ?`, [terminalId]);
            if (!existingTerminal) {
                throw new common_1.NotFoundException('Terminal not found');
            }
            if (existingTerminal.terminal_image) {
    ...
    async checkTerminalCodeExists(terminalCode, companyId) {
    ...
    async findStoresByCompanyAndMerchant(companyId, merchantId) {
        try {
    ...
    async findTerminalsByIds(terminalIds) {
        try {
    ...
    async findTerminalsBySerial(dto) {
        try {
            let query = `SELECT * FROM io_terminal WHERE serial_number = ?`;
            const params = [dto.serial_number];
            if (dto.company_id) {
                query += ` AND company_id = ?`;
                params.push(dto.company_id);
            }
    ...
    async findTerminalsBySim(dto) {
        try {
            let query = `SELECT * FROM io_terminal WHERE sim_number = ?`;
            const params = [dto.sim_number];
            if (dto.company_id) {
                query += ` AND company_id = ?`;
                params.push(dto.company_id);
            }
    ...
    async findTerminalsByExpireDate(dto) {
        try {
    ...
    async updateTerminalApproval(terminalId, approvalDto) {
        try {
...
exports.IoTerminalService = IoTerminalService;
exports.IoTerminalService = IoTerminalService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [typeorm_1.DataSource])
], IoTerminalService);
//# sourceMappingURL=ioterminal.service.js.map