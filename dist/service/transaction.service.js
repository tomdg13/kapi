...
exports.TrasactionService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("typeorm");
let TrasactionService = class TrasactionService {
    constructor(dataSource) {
        this.dataSource = dataSource;
    }
    async generateUniqueRRN() {
        let rrn;
        let isUnique = false;
        let attempts = 0;
        ...
        while (!isUnique && attempts < maxAttempts) {
            ...
            try {
                const existingRRN = await this.dataSource.query('SELECT rrn FROM kd_txn WHERE rrn = ? LIMIT 1', [rrn]);
    ...
    async createStatementEntryWithQueryRunner(queryRunner, phone, pointDebit, pointCredit, newBalance) {
        try {
            const statementSql = `
        INSERT INTO kd_sttm (
    ...
    async addTransaction(dto) {
      ...
      `;
            ...
            await this.createStatementEntryWithQueryRunner(queryRunner, phone, pointDebit, pointCredit, newBalance);
            if (txn_type === 'TRANSFER' && phone_to) {
                ...
                await this.createStatementEntryWithQueryRunner(queryRunner, phone_to, 0, pointValue, receiverNewBalance);
            }
    ...
    async findTransactionsByDateRange(phone, dateFrom, dateTo) {
        try {
            const query = `
        SELECT t.*, tt.txntype, tt.txntype_la 
        FROM kd_txn t
    ...
    async getCustomerStatement(phone, limit = 50) {
        try {
            const query = `
        SELECT 
          sttm_id,
    ...
    async getStatementByDateRange(phone, dateFrom, dateTo) {
        try {
            const query = `
        SELECT 
          sttm_id,
    ...
    async getCurrentBalanceDetailed(phone) {
        try {
    ...
    async migrateTransactionsToStatements() {
        try {
    ...
    async getTransactionSummary(phone) {
        try {
            let query = `
        SELECT 
    ...
    async getSystemHealth() {
        try {
            ...
            const recentActivity = await this.dataSource.query('SELECT COUNT(*) as count FROM kd_txn WHERE created_at >= DATE_SUB(NOW(), INTERVAL 24 HOUR)');
            return {
                status: 'healthy',
...
exports.TrasactionService = TrasactionService;
exports.TrasactionService = TrasactionService = __decorate([
