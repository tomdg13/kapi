...
exports.SettlementDetailsService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("typeorm");
let SettlementDetailsService = class SettlementDetailsService {
    constructor(dataSource) {
        this.dataSource = dataSource;
    }
    async checkDuplicates(company_id, system_transaction_ids) {
        try {
            ...
            const existingRecords = await this.findBySystemTransactionIds(company_id, system_transaction_ids);
            const duplicateIds = existingRecords.map((record) => record.system_transaction_id);
    ...
    async createFromCSV(csvData, sourceFilename) {
        try {
            const results = [];
            const errors = [];
            let skippedRows = 0;
    ...
    async update(id, dto) {
        try {
            const existingRecord = await this.dataSource.query('SELECT id FROM settlement_details WHERE id = ?', [id]);
            if (existingRecord.length === 0) {
                throw new common_1.NotFoundException(`Settlement detail with ID ${id} not found`);
            }
            const updateFields = [];
            const params = [];
    ...
    async findById(dto) {
        try {
            ...
            if (result.length === 0) {
                throw new common_1.NotFoundException(`Settlement detail with ID ${dto.id} not found`);
            }
            return {
                status: 'success',
    ...
    async getApiCodeStats(company_id, filters) {
        try {
            let query = `
        SELECT 
          api_code,
    ...
    async findBySystemTransactionIds(company_id, system_transaction_ids) {
        try {
            if (!system_transaction_ids || system_transaction_ids.length === 0) {
                return [];
            }
...
exports.SettlementDetailsService = SettlementDetailsService;
exports.SettlementDetailsService = SettlementDetailsService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [typeorm_1.DataSource])
], SettlementDetailsService);
//# sourceMappingURL=settlement_details.service.js.map