...
exports.IoroleService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("typeorm");
let IoroleService = class IoroleService {
    ...
    async getRoleByCode(role_code, company_id) {
        try {
            let query = `
        SELECT 
          role_id,
    ...
    async createRole(roleData) {
        try {
            const { role_name, role_code, description, company_id, level, permissions, status, is_system, created_by, } = roleData;
    ...
    async updateRole(role_id, roleData) {
        try {
            ...
            const [existingRole] = await this.dataSource.query(`SELECT role_id, role_name, role_code, is_system FROM io_role WHERE role_id = ?`, [role_id]);
            if (!existingRole) {
    ...
    async deleteRole(role_id) {
        try {
            ...
            const [existingRole] = await this.dataSource.query(`SELECT role_id, role_name, role_code, is_system, status FROM io_role WHERE role_id = ?`, [role_id]);
            if (!existingRole) {
                console.log('❌ Role not found');
                throw new common_1.HttpException({
                    status: 'error',
            ...
            const usersWithRole = await this.dataSource.query(`SELECT COUNT(*) as count FROM io_user WHERE role_id = ? AND status != 'delete'`, [role_id]);
            ...
            await this.dataSource.query(`UPDATE io_role SET status = 'inactive', updated_at = NOW() WHERE role_id = ?`, [role_id]);
    ...
    async hardDeleteRole(role_id) {
        try {
            console.log('⚠️  HARD DELETE - Role ID:', role_id);
            const [existingRole] = await this.dataSource.query(`SELECT role_id, role_name, is_system FROM io_role WHERE role_id = ?`, [role_id]);
            if (!existingRole) {
                throw new common_1.HttpException({
                    status: 'error',
                    message: `Role with ID ${role_id} not found`,
            ...
            await this.dataSource.query(`DELETE FROM io_role WHERE role_id = ?`, [role_id]);
    ...
    async restoreRole(role_id) {
        try {
            console.log('🔄 Restoring role ID:', role_id);
            const [existingRole] = await this.dataSource.query(`SELECT role_id, role_name, status FROM io_role WHERE role_id = ?`, [role_id]);
            if (!existingRole) {
                throw new common_1.HttpException({
                    status: 'error',
                    message: `Role with ID ${role_id} not found`,
            ...
            await this.dataSource.query(`UPDATE io_role SET status = 'active', updated_at = NOW() WHERE role_id = ?`, [role_id]);
    ...
    async getRolePermissions(role_id) {
        try {
            ...
            const [existingRole] = await this.dataSource.query(`SELECT role_id, role_name FROM io_role WHERE role_id = ?`, [role_id]);
            if (!existingRole) {
                console.log('❌ Role not found');
                throw new common_1.HttpException({
                    status: 'error',
    ...
    async updateRolePermissions(role_id, permission_ids) {
        ...
        try {
            ...
            const [existingRole] = await queryRunner.query(`SELECT role_id, role_name, is_system FROM io_role WHERE role_id = ?`, [role_id]);
            if (!existingRole) {
                console.log('❌ Role not found');
                throw new common_1.HttpException({
                    status: 'error',
            ...
            if (permission_ids && permission_ids.length > 0) {
                ...
                const validPermissions = await queryRunner.query(`SELECT permission_id FROM io_permission WHERE permission_id IN (${placeholders})`, permission_ids);
            ...
            await queryRunner.query(`DELETE FROM io_role_permission WHERE role_id = ?`, [role_id]);
            console.log('🗑️  Removed existing permissions');
            if (permission_ids && permission_ids.length > 0) {
                const insertValues = permission_ids.map((permission_id) => `(${role_id}, ${permission_id})`).join(', ');
                const insertQuery = `
        ...
        `;
            ...
            await queryRunner.query(`UPDATE io_role SET updated_at = NOW() WHERE role_id = ?`, [role_id]);
    ...
    async addPermissionsToRole(role_id, permission_ids) {
        try {
            ...
            const existingPermissions = await this.dataSource.query(`SELECT permission_id FROM io_role_permission WHERE role_id = ?`, [role_id]);
            ...
            const insertValues = newPermissionIds.map((permission_id) => `(${role_id}, ${permission_id})`).join(', ');
            const insertQuery = `
        INSERT INTO io_role_permission (role_id, permission_id)
        VALUES ${insertValues}
      `;
    ...
    async removePermissionsFromRole(role_id, permission_ids) {
        try {
            ...
            const placeholders = permission_ids.map(() => '?').join(',');
            const result = await this.dataSource.query(`DELETE FROM io_role_permission WHERE role_id = ? AND permission_id IN (${placeholders})`, [role_id, ...permission_ids]);
    ...
    async copyRolePermissions(source_role_id, target_role_id, replace = false) {
        try {
            console.log('📋 Copying permissions from role', source_role_id, 'to role', target_role_id);
            const roles = await this.dataSource.query(`SELECT role_id, role_name, is_system FROM io_role WHERE role_id IN (?, ?)`, [source_role_id, target_role_id]);
            if (roles.length !== 2) {
                throw new common_1.HttpException({
                    status: 'error',
            ...
            const sourcePermissions = await this.dataSource.query(`SELECT permission_id FROM io_role_permission WHERE role_id = ?`, [source_role_id]);
    ...
    async checkUserModuleAccess(user_id, module_code) {
        try {
            ...
            const [user] = await this.dataSource.query(`SELECT role_id, name FROM io_user WHERE user_id = ? AND status != 'delete'`, [user_id]);
            if (!user) {
    ...
    async checkIsSuperAdmin(user_id) {
        try {
            const [user] = await this.dataSource.query(`SELECT role_id FROM io_user WHERE user_id = ? AND status != 'delete'`, [user_id]);
            if (!user) {
                return {
                    status: 'error',
                    message: 'User not found',
...
exports.IoroleService = IoroleService;
exports.IoroleService = IoroleService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [typeorm_1.DataSource])
], IoroleService);
//# sourceMappingURL=iorole.service.js.map