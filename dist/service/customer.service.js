...
let customerService = class customerService {
    ...
    async findCustomersByRole(dto) {
        try {
            ...
            const customersWithPhotos = result.map((customer) => ({
                ...customer,
    ...
    async findDistrictsByProvinceId(pr_id) {
        try {
            const result = await this.dataSource.query('SELECT * FROM kd_district WHERE pr_id = ? order by dr_name', [pr_id]);
            return {
                status: 'success',
    ...
    async addCustomerWithPhoto(customerDto) {
        try {
            const { name, username, email, password, phone, document_id, photo, photo_id, village_id, district_id, province_id, status, role, account_bank_id, account_no, account_name, language, bio, } = customerDto;
    ...
    async updateCustomerWithPhoto(phone, customerDto) {
        try {
            console.log('📝 Received customerDto:', JSON.stringify(customerDto, null, 2));
            const { name, username, email, password, phone: newPhone, document_id, photo, photo_id, village_id, district_id, province_id, status, role, account_bank_id, account_no, account_name, language, bio, } = customerDto;
            const [existingCustomer] = await this.dataSource.query(`SELECT photo, photo_id FROM kd_customer WHERE phone = '${phone}'`);
            if (!existingCustomer) {
    ...
    async updateCustomerStatus(phone, onlineStatus) {
        try {
            const result = await this.dataSource.query(`UPDATE kd_customer SET status = ? WHERE phone = ?`, [onlineStatus, phone]);
    ...
    async updateCustomeronStatus(phone, onlineStatus) {
        try {
            const result = await this.dataSource.query(`UPDATE kd_customer SET online = ? WHERE phone = ?`, [onlineStatus, phone]);
            if (result.affectedRows === 0) {
                throw new Error('Customer not found or no change');
            }
            return {
    ...
    async updateCustomerOnlineStatus(phone, onlineStatus) {
        try {
            const result = await this.dataSource.query(`UPDATE kd_driver SET online = '${onlineStatus}' WHERE phone = '${phone}'`);
            if (result.affectedRows === 0) {
    ...
    async addDriverWithPhoto(driverDto) {
        try {
            const { name, username, email, password, phone, document_id, photo, photo_id, village_id, district_id, province_id, status, role, account_bank_id, account_no, account_name, language, bio, online, } = driverDto;
    ...
    async saveDriverImage(base64Str) {
        if (!base64Str)
            return null;
        try {
  ...
  `;
    ...
    async updateCustomerPassword(phone, newPassword) {
        try {
            const result = await this.dataSource.query(`UPDATE kd_customer SET password = '${newPassword}' WHERE phone = '${phone}'`);
            if (result.affectedRows === 0) {
                throw new Error('Customer not found or no change');
            }
            return {
    ...
    async updateDriverPassword(phone, newPassword) {
        try {
            const result = await this.dataSource.query(`UPDATE kd_driver SET password = '${newPassword}' WHERE phone = '${phone}'`);
            if (result.affectedRows === 0) {
                throw new Error('Customer not found or no change');
            }
            return {
    ...
    async updateioPassword(phone, newPassword) {
        try {
            const result = await this.dataSource.query(`UPDATE io_user SET password = '${newPassword}',status = 'active' WHERE phone = '${phone}'`);
            if (result.affectedRows === 0) {
                throw new Error('Customer not found or no change');
            }
            return {
    ...
    async getNearbyPromotes(dto) {
        try {
            ...
            const limit = limitResult[0]?.setup || 10;
            const query = `
      SELECT 
        promote_id,
        promote_note,
    ...
    async addPromotionWithPhoto(promoteDto) {
        try {
            const { promote_note, promote_photo, promote_phone, location, } = promoteDto;
    ...
    async savePromoteImage(base64Str) {
        if (!base64Str)
            return null;
        try {
    ...
    async addbannerWithPhoto(bannerDto) {
        try {
            console.log('📥 Received banner data:', bannerDto);
            const { banner_note, banner_photo, banner_index, banner_status, banner_link, } = bannerDto;
    ...
    async saveBannerImage(base64Str) {
        if (!base64Str)
            return null;
        try {
    ...
    async putUpdateBanner(id, bannerDto) {
        try {
            ...
            const values = [banner_note, banner_index, banner_status, banner_link, id];
            let sql = `
      UPDATE kd_banner 
    ...
    async getCustomerWithBalance(customerId, phone) {
        try {
            let whereClause = '';
            const params = [];
            if (customerId) {
    ...
    async getAllCustomersWithBalance(limit = 50, offset = 0) {
        try {
            const query = `
        SELECT 
          c.customer_id,
    ...
    async getCustomerLeaderboard(limit = 20) {
        try {
            const query = `
        SELECT 
          c.customer_id,
    ...
    async searchCustomers(searchTerm, searchBy = 'all') {
        try {
            let whereClause = '';
            const params = [];
